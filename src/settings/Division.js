...
export default function Location() {
  ...
  const[uniqueId,setUniqueId] = useState(params.divisionId);
  ...
  const [editLabel, setEditLabel] = useState('+Add New')
    ...
    useEffect(() => {
      const taskColRef1 = collection(db, 'divisions');
...
//      const taskColRef = query(collection(db, 'books'), orderBy('created', 'desc'))
  ...
  const handleUpdate = async () => {
    ...
    if(uniqueId === 'Add New'){
      ...
      const locationsRef = doc(db, 'divisions', locationsRefDoc);
      ...
      const locationsUpdateRef = doc(db, 'divisions', id);
...
const handleDelete = async () => {
  ...
  if(isExecuted == false)
    return
  const taskDocRef = doc(db, 'divisions', id)
  try{
...
return (
    ...
    <main style={{ padding: "1rem" }}>
      ...
      <p>
        ...
        <button
          onClick={() => {
            navigate("/divisions" + mlocation.search);
          }}
        >
           Done
        </button>        
